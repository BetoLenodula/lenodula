<?php
	...
	class WordCloud
	{
		...
		public function __construct($words = false, $idg = 0)
		{	
			$this->id_get = $idg;
			...
			if (is_array($this->wordsArray)) {
				...
				foreach ($this->wordsArray as $word => $arrayInfo) {
					...
					if ($returnType == 'array') {
						...
						if($this->id_get != 0){
							$return .= "<a class='underl' href='/temas/searchtag/{$arrayInfo['word']}?g={$this->id_get}'><span class='word size{$sizeRange}'>&nbsp;{$arrayInfo['word']}&nbsp;</span></a>";
						}
						else{
							if(!empty($arrayInfo['word'])){
								$return .= "<a class='underl' href='/comentarios/destacar/{$arrayInfo['word']}'><span class='word smsize{$sizeRange}'>&nbsp;#{$arrayInfo['word']}&nbsp;</span></a>";	
							}
							else{
								$return = '';
							}
