...
$('#insert_words #words').change(function(){
    txt = this.value;
    if(txt.length > 12){
      msgAlert('Elige palabras con menos de 12 letras!');
      cerrar_alert_focus('#words');
    }
    else{
...
$('#send_cwrfnd').click(function(){
  ...
  argumento = url[5];
   ...
   var nom = $('#nombre').val();
   var wrs = $('#tags_hidden').val();

   var exprNomWF = /^[0-9a-zA-ZñÑáéíóúÁÉÍÓÚ\s\,\(\)\:\"\+\-\*\/\;\.\_\¿\?\¡\!\&\-\$\=\%\']{1,100}$/;
   var exprTags = /^[0-9a-zA-ZñÑáéíóúÁÉÍÓÚ\,]{1,300}$/;

    if(nom == ''){
        msgAlert('Indica un nombre para el título!!');
    ...
    if(!exprNomWF.test(nom)){
        msgAlert('El nombre NO debe tener caracteres extraños!!');
        cerrar_alert_focus('#nombre');
        return false;   
    }
     
    if(wrs == ''){
        msgAlert('Agrega palabras!!');
        cerrar_alert_focus('#words');
        return false;
    }
    if(wrs.length > 300){
        msgAlert('Son demasiadas palabras!!');
        cerrar_alert_focus('#words');
        return false;
    }
    if(!exprTags.test(wrs)){
        msgAlert('No utilices caracteres extraños ni ESPACIOS, solamente palabras');
        cerrar_alert_focus('#words');
        return false;
    }

    dat = $('#frmWordFind').serializeArray();
    ...
    $.ajax({
      url: '/temas/crearwordfind/'+argumento,
      type: 'post',
      data: dat
    }).done(function(info){
      if(info == false){
        ...
        location.replace('/temas/wordfind/'+info);
      }
       $('.spinner').hide();
